...
import type { WorkflowActor } from "./corporate-actions-v2";
...
export async function createCaseFromIntakeDraft(id: string, actor: WorkflowActor): Promise<Record<string, any>> {
  ...
  const event = {
    ...
    positions: [],
    options: voluntary ? [{ id: "pending-election", label: "Election terms pending", description: "Confirm available election options from the validated notice.", result: "No instruction can be produced yet.", default: true, fundingFormula: "Not available" }] : [],
    instruction: { status: voluntary ? "DRAFT" : "Not required", destination: "N/A", reference: "N/A", generatedAt: "", content: "No external instruction is generated by this intake workflow.", simulated: false, approvalActor: "" },
    ...
    audit: [{ id: `audit-${eventId}`, eventId, action: "Case created from validated source", actor: actor.name, actorId: actor.id, actorRole: actor.role, actorType: "user", timestamp: new Date().toISOString(), detail: "Intake source and extracted terms were validated before the case was created.", previousValue: "", newValue: "Validated", reason: "", evidenceId: draft.id, workflowStatus: "Validated" }],
  ...
  draft.status = "Case created";
  draft.caseId = event.id;
  draft.updatedAt = new Date().toISOString();
  draft.audit.unshift({ timestamp: draft.updatedAt, action: "Case created", actor: actor.name, detail: event.reference });
  await persistDraft(draft);
  return event;
}
