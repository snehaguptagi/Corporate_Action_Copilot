...
const sum = (items, key) => round(items.reduce((total, item) => total + Number(item[key] ?? 0), 0));
const notice = (documentName, excerpt, pages, source = "Synthetic custodian portal") => ({
    documentName,
    source,
...
const term = (key, label, value, page, evidence, reviewStatus = "Validated", confidence = 0.97) => ({
    key,
    label,
    value,
    page: `p. ${page}`,
...
const position = (id, fund, account, isin, eligibleQuantity, positionDate, eligibilityStatus = "Eligible", dataQualityWarning = "") => ({
    id,
    fund,
    account,
    isin,
...
const audit = (eventId, action, detail, actor, workflowStatus, extras = {}) => ({
...
const task = (id, eventId, eventReference, title, owner, due, priority, category, detail, status = "Open", dependency = "", sourceRule = "CA-CONTROL-001") => ({
    id,
    eventId,
    eventReference,
    title,
...
const preloadedEvents = [
    eventBase({
        ...
        notice: notice("cash-dividend-notice.pdf", "The Board has declared an interim cash dividend. The payment currency is defined in the settlement section.", [
...
function heroRightsEvent(documentName, source, actor) {
    ...
    return eventBase({
        ...
        notice: {
            ...notice(documentName, "The uploaded synthetic notice is classified as a voluntary rights issue. Key terms remain unvalidated until an analyst reviews the evidence.", [
...
export function appendAudit(event, action, detail, actor = "System", extras = {}) {
...
function syncCalculationInput(event, key, value) {
    event.calculationInputs ??= {};
    if (["rightsRatio", "bonusRatio", "splitRatio"].includes(key)) {
        const parsed = ratio(value);
        if (!parsed || parsed[0] <= 0 || parsed[1] <= 0)
            throw new Error(`${key} must use a positive ratio such as “1 for 5”.`);
        if (key === "splitRatio")
            event.calculationInputs.splitFactor = parsed[0] / parsed[1];
        else {
            event.calculationInputs.ratioNumerator = parsed[0];
            event.calculationInputs.ratioDenominator = parsed[1];
        }
    }
    const valueMap = { subscriptionPrice: "subscriptionPrice", rate: "rate", offerPrice: "offerPrice", cashRate: "cashRate", shareExchangeRatio: "shareExchangeRatio", maximumAcceptance: "maximumPercentage" };
    if (valueMap[key]) {
    ...
    if (["recordDate", "paymentDate", "settlementDate", "marketDeadline"].includes(key)) {
        ...
        if (Number.isNaN(date.getTime()))
            throw new Error(`${key} must contain a valid date or date-time.`);
        if (key === "recordDate")
            event.calculationInputs.recordDate = date.toISOString().slice(0, 10);
    }
}
export function missingRequiredTerms(event) {
    return (event.requiredTermKeys ?? []).filter((key) => {
        const found = event.terms?.find((candidate) => candidate.key === key);
        return !found || !found.value?.trim() || found.reviewStatus !== "Validated";
    });
}
export function refreshValidation(event) {
    const missingTerms = missingRequiredTerms(event);
    event.validation = { missingTerms, isReady: missingTerms.length === 0 };
}
export function applyTermUpdates(event, updates, actor, reason) {
    if (actor.role !== "Operations Analyst")
        throw new Error("Only an Operations Analyst can validate or correct extracted terms.");
    for (const update of updates) {
        const current = event.terms?.find((candidate) => candidate.key === update.key);
        if (!current)
            throw new Error(`Unknown extracted term: ${update.key}`);
        const oldValue = current.value;
        const changed = oldValue !== update.value;
        const correctionReason = update.reason ?? reason ?? "";
        if (changed && !correctionReason.trim())
            throw new Error(`A correction reason is required for ${current.label}.`);
...
export function calculateEventImpacts(event, actor) {
    if (actor.role !== "Operations Analyst")
        throw new Error("Only an Operations Analyst can run deterministic calculations.");
    if (event.impacts?.some((impact) => impact.electionDecision))
        throw new Error("Calculation cannot be re-run after elections are submitted. Return the event for analyst review first.");
    if (["Approved", "Awaiting settlement", "Reconciled", "Break identified", "Closed"].includes(event.status))
        throw new Error(`Calculation cannot be re-run while the event is ${event.status}.`);
    refreshValidation(event);
    if (!event.validation.isReady)
        throw new Error(`Calculation is blocked until these terms are validated: ${event.validation.missingTerms.join(", ")}.`);
    const inputs = event.calculationInputs ?? {};
    const positions = eligiblePositions(event);
    if (positions.length === 0)
        throw new Error("Calculation is blocked because no eligible positions were found.");
    event.impacts = positions.map((item) => {
        const common = {
        ...
        if (event.eventType === "Cash dividend") {
            ...
            return { ...common, formula: `${item.eligibleQuantity.toLocaleString()} × ${event.terms.find((term) => term.key === "rate").value}`, expected: expectedCash, expectedCash, expectedSecurityQuantity: 0, securityMovement: "Cash receipt", currency: inputs.currency ?? event.currency };
        }
...
export function recordElection(event, body, actor) {
    if (actor.role !== "Operations Analyst")
        throw new Error("Only an Operations Analyst can prepare an election.");
    if (event.processingType === "Mandatory")
        throw new Error("Mandatory events do not have an election workflow.");
    if (!["Election required", "Awaiting approval"].includes(event.status))
        throw new Error(`Election cannot be saved while the event is ${event.status}.`);
    const impact = event.impacts?.find((candidate) => candidate.id === body.impactId);
    const option = event.options?.find((candidate) => candidate.id === body.optionId);
    if (!impact || !option)
        throw new Error("Impact or election option is invalid.");
    const maximum = Number(impact.entitlement ?? impact.eligibleQuantity);
    if (!Number.isFinite(body.quantityElected) || body.quantityElected < 0 || body.quantityElected > maximum)
        throw new Error(`Election quantity must be between 0 and ${maximum.toLocaleString()}.`);
    ...
    event.reconciliation.expectedCash = round(event.impacts.reduce((total, current) => (total + Number(current.electionDecision?.requiredFunding ?? 0)), 0));
    event.reconciliation.expected = event.reconciliation.expectedCash;
    event.reconciliation.expectedSecurityQuantity = round(event.impacts.reduce((total, current) => (total + (current.election === "exercise" ? Number(current.electionDecision?.quantityElected ?? 0) : 0)), 0), 3);
    event.reconciliation.note = "Expected settlement is derived from the recorded election quantities and funding.";
    event.status = event.impacts.every((candidate) => candidate.election) ? "Awaiting approval" : "Election required";
    event.settlementStage = event.status;
    for (const currentTask of event.tasks ?? [])
        if (currentTask.category === "Election")
...
export function approveControlledEvent(event, approved, note, actor) {
    if (actor.role !== "Reviewer")
        throw new Error("Only a Reviewer can approve or return controlled actions.");
    if (event.processingType !== "Mandatory" && event.status !== "Awaiting approval")
        throw new Error(`Approval is blocked while the event is ${event.status}.`);
    if (approved) {
        const makerActions = new Set(["Election submitted", "Extracted term corrected", "Extracted term validated"]);
        const makerConflict = (event.audit ?? []).some((entry) => makerActions.has(entry.action) && entry.actorId === actor.id)
            || event.impacts.some((impact) => impact.electionDecision?.analystId === actor.id);
        if (makerConflict)
            throw new Error("Maker-checker control failed: the person who prepared an election cannot approve it.");
    }
    event.impacts.forEach((impact) => {
        impact.approval = approved ? "Approved" : "Returned";
        if (impact.electionDecision)
            impact.electionDecision.status = approved ? "Approved" : "Returned";
    });
...
export function simulateInstruction(event, status, actor) {
    if (actor.role !== "Operations Analyst")
        throw new Error("Only an Operations Analyst can prepare a simulated instruction.");
    const canIssue = event.processingType === "Mandatory" ? ["Instruction ready", "Approved"].includes(event.status) : event.status === "Approved";
    if (!canIssue)
        throw new Error(`Instruction is blocked while the event is ${event.status}. Approval and calculation controls must complete first.`);
    if (status !== "SIMULATED — NOT SENT")
        throw new Error("The POC only supports the explicit status SIMULATED — NOT SENT.");
    const electionLines = event.impacts.map((impact) => `${impact.account}: ${impact.electionDecision?.optionLabel ?? "Mandatory processing"}; quantity ${impact.electionDecision?.quantityElected ?? impact.expectedSecurityQuantity ?? 0}`).join("\n");
    event.instruction = {
        status,
        ...
        generatedAt: now(),
        simulated: true,
        approvalActor: event.audit?.find((entry) => entry.action === "Checker approval recorded")?.actor ?? "",
...
export function reconcileEvent(event, body, actor) {
    if (!["Operations Analyst", "Operations Manager"].includes(actor.role))
        throw new Error("Only Operations Analysts or Managers can record settlement results.");
    if (!["Awaiting settlement", "Break identified"].includes(event.status))
        throw new Error(`Settlement reconciliation is blocked while the event is ${event.status}. A simulated instruction is required first.`);
    ...
    if (actualCurrency !== recon.expectedCurrency)
        classification = "Wrong currency";
    else if (recon.expectedAccount !== "Multiple accounts" && actualAccount !== recon.expectedAccount)
        classification = "Wrong account";
    else if (actualCash === 0 && actualSecurityQuantity === 0)
        classification = "Missing";
    else if (cashDifference < -recon.tolerance || securityDifference < -recon.tolerance)
        classification = "Under-settled";
    else if (cashDifference > recon.tolerance || securityDifference > recon.tolerance)
        classification = "Over-settled";
    else if (actualSettlementDate !== recon.expectedSettlementDate)
    ...
    if (classification === "Matched") {
        ...
        const exists = event.tasks?.some((current) => current.category === "Reconciliation" && current.status === "Open");
        if (!exists)
            event.tasks.push(task(`task-${event.id}-break`, event.id, event.reference, "Investigate settlement difference", "Aisha Mehta", "Today · 16:00", "High", "Reconciliation", `${classification}: expected and actual settlement results differ.`, "Open", "", "CA-CONTROL-007"));
    }
    event.settlementStage = event.status;
...
export async function ensureCorporateActionSeedData() {
    if (!seedPromise) {
        seedPromise = (async () => {
            ...
            if (existing && existing.data.seedVersion === SEED_VERSION)
                return;
            if (existing)
                await db.delete(corporateActionEventsTable);
...
export async function getCorporateActionEvents() {
...
export async function getCorporateActionEvent(id) {
    await ensureCorporateActionSeedData();
    const { corporateActionEventsTable, db } = await import("@workspace/db");
...
export async function saveCorporateActionEvent(event) {
    const { corporateActionEventsTable, db } = await import("@workspace/db");
    await db.update(corporateActionEventsTable).set({ data: event }).where(eq(corporateActionEventsTable.id, event.id));
    return clone(event);
}
export async function createIntakeEvent(fileName, source, actor) {
    await ensureCorporateActionSeedData();
    if (!fileName.toLowerCase().includes("rights"))
        throw new Error("This POC intake accepts the supplied synthetic rights-issue notice. Choose a filename containing “rights”.");
